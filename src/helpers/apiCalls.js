...
export async function fetchData(lat, lon) {
    ...
    try {
        const predictiveWeatherData = await axios.get(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&appid=${API_KEY}&lang=nl`);
...
export async function fetchDailyData(lat, lon) {
    ...
    try {
        const weatherData = await axios.get(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${API_KEY}&lang=nl`);
...
export async function fetchLocationData(searchCity) {
    ...
    try {
        let LocationResult = await axios.get(`http://api.openweathermap.org/geo/1.0/direct?q=${searchCity},NL&appid=${API_KEY}`);
